...
public abstract class BNSoundProvider extends SoundDefinitionsProvider {

    protected BNSoundProvider(PackOutput output, ExistingFileHelper helper) {
        super(output, BountifulNpcMod.MOD_ID, helper);
    }
    ...
    public void generateNewSound(RegistryObject<SoundEvent> event, String baseSoundDirectory, int numberOfSounds, boolean subtitle) {
        ...
        if (subtitle) {
            ...
            definition.subtitle("subtitles." + BountifulNpcMod.MOD_ID + "." + splitSoundName[0] + "." + splitSoundName[2]);
        }
        for (int i = 1; i <= numberOfSounds; i++) {
            definition.with(SoundDefinition.Sound.sound(ResourceLocation.fromNamespaceAndPath(BountifulNpcMod.MOD_ID, baseSoundDirectory + (numberOfSounds > 1 ? "_" + i : "")), SoundDefinition.SoundType.SOUND));
        }
    ...
    public void makeMusicDisc(RegistryObject<SoundEvent> event, String discName) {
        this.add(event, SoundDefinition.definition()
                .with(SoundDefinition.Sound.sound(ResourceLocation.fromNamespaceAndPath(BountifulNpcMod.MOD_ID, "music/" + discName), SoundDefinition.SoundType.SOUND)
                        .stream()));
    }
